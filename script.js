...
function renderremoveflavor(ingredient) {
  ...
  function renderLayer(){
      ...
      for(let i=0; i<layer.length;i++){ layer[i].style.display = "none";}

      for(let i =0; j=4; i<flavour.length;i++;j--){
  ...
  function renderBillBoard(){
      ...
      for(const property in state) {
          if(state[property]===false){element[i].style.display="inherit";}
  ...
  function renderPrice() {
      var total = 0;
      for(let i=0; i<flavor.length;i++) { total = total + price[flavor[i]];
    }
